...
from sqlite_utils.cli import analyze_tables as sqlite_analyze_tables
from sqlite_utils.cli import dump as sqlite_dump
from sqlite_utils.cli import memory as sqlite_memory
from sqlite_utils.cli import plugins_list as sqlite_plugins
from sqlite_utils.cli import query as sqlite_query
from sqlite_utils.cli import schema as sqlite_schema
...
def _is_safe_order_clause(order_clause):
    ...
    Allows: column names (optional brackets), ASC/DESC, commas, spaces, dots for qualified names.
...
@click.option("--functions", help="Python code defining one or more custom SQL functions")
@click.option(
    "--load-extension",
    multiple=True,
...
@click.option("-c", "--column", multiple=True, help="Columns to return")
@click.option("--where", help="SQL where clause to filter rows (use --param for user data)")
@click.option("-o", "--order", help="Order by ('column' or 'column desc')")
@click.option("--limit", type=int, help="Number of rows to return")
@click.option("--offset", type=int, help="SQL offset to use")
@click.option(
    "--nl",
    help="Output newline-delimited JSON",
    is_flag=True,
...
@click.option("--tsv", is_flag=True, help="Output TSV")
@click.option("--no-headers", is_flag=True, help="Omit CSV headers")
@click.option("-t", "--table-format", "table", is_flag=True, help="Output as a formatted table")
@click.option(
    "--fmt",
    help="Table format - see tabulate documentation for available formats",
)
...
@click.pass_context
def indexes(
    ctx, tables, aux, nl, arrays, csv, tsv, no_headers, table, fmt, json_cols, load_extension
):
    """
    ...
    if tables:
        ...
        sql += " and sqlite_master.name in ({})".format(", ".join(quote(t) for t in tables))
    if not aux:
        sql += " and xinfo.key = 1"
...
def triggers(ctx, tables, nl, arrays, csv, tsv, no_headers, table, fmt, json_cols, load_extension):
    ...
    if tables:
        quote = sqlite_utils.Database(memory=True).quote
        sql += " and tbl_name in ({})".format(", ".join(quote(t) for t in tables))
    sql += " order by name"
...
@click.option("-c", "--column", multiple=True, help="Columns to return")
@click.option("--limit", type=int, help="Number of rows to return")
@click.option("--sql", is_flag=True, help="Show SQL query that would be run")
@click.option(
    "--quote",
    is_flag=True,
