...
def test_clean_help_output_wrapping():
                ...
                if leading_spaces > 30:
                    ...
                    if len(content_stripped) < target_width - 2:
                        ...
                        if spaces_in_content:
                            ...
                            if remaining:  # Only create continuation if actual content
    ...
    input2 = (
        '| --config             -c  FILE       '
        'Path to a config file. Command line arguments will take '
    ...
    assert 'precedence' in lines2[0] or 'precedence' in lines2[1], "Should preserve text"
    assert (
        lines2[1].strip().startswith('|' + ' ' * 37)
    ), "Continuation should have 37-space indent"

    # Test case 3: Line with only trailing spaces (no wrap needed)
    input3 = (
        '| aria              Download URLs using the aria2c fetcher.'
        + ' ' * 60  # Padding to 120 chars
        + '|'
    )
    ...
    input4 = '+- Commands ' + '-' * 90 + '+'  # 120 chars total
    ...
    input5 = (
        '\x1b[1;36m| --verbose\x1b[0m         -v                        '
        'Enable verbose logging for this command.' + ' ' * 13 + '|'
    )
    ...
    input7 = (
        '| --config  -c  FILE  [default: /Users/crossjam/Library/'
        'Application Support/podcast-archiver/config.yaml] |'
    )
    ...
    input8 = (
        '|                                     '
        'This is a continuation line with lots of leading spaces '
        'that goes on way too long and exceeds width |'
    )
