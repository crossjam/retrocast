...
The `configure` command group manages the retrocast configuration directory and database.
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["configure", "archive", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli configure archive [OPTIONS]
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["configure", "check", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli configure check [OPTIONS]
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["configure", "initialize", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli configure initialize [OPTIONS]
...
```bash
retrocast configure initialize
```
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["configure", "location", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli configure location [OPTIONS]
  ...
  Examples:     retrocast configure location              # Table view
  retrocast configure location --format json  # All paths as JSON     retrocast
  configure location --db-path    # Just database path     retrocast configure
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["configure", "reset-db", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli configure reset-db [OPTIONS]
