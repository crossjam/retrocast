...
Examples:
  ...
  # Query the database
  scrobbledb query query "SELECT * FROM tracks LIMIT 10"

  # List all tables with row counts
  scrobbledb query tables --counts

  # View table schema
  scrobbledb query schema tracks

  # Browse table data
  scrobbledb query rows plays --limit 20

  # Use a different database
  scrobbledb query query "SELECT * FROM users" --database /path/to/other.db
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "analyze-tables", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query analyze-tables [OPTIONS] [TABLES]...

  Analyze the columns in one or more tables.

  Example:

          scrobbledb query analyze-tables tracks
          scrobbledb query analyze-tables tracks -c artist_name
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "dump", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query dump [OPTIONS]

  Output a SQL dump of the schema and full contents of the database.

  Example:

          scrobbledb query dump > backup.sql
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "indexes", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query indexes [OPTIONS] [TABLES]...

  Show indexes for the whole database or specific tables.

  Example:

          scrobbledb query indexes
          scrobbledb query indexes tracks
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "memory", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query memory [OPTIONS] [PATHS]... SQL_QUERY
  ...
  Example:
          ...
          scrobbledb query memory data.csv "SELECT * FROM data LIMIT 10"
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "plugins", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query plugins [OPTIONS]

  List installed sqlite-utils plugins.

  Example:

          scrobbledb query plugins
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "query", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query query [OPTIONS] SQL_QUERY

  Execute SQL query and return the results as JSON.

  Example:

          scrobbledb query query "SELECT * FROM tracks WHERE artist_id = :id LIMIT 10" -p id 123
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "rows", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query rows [OPTIONS] TABLE_NAME
  ...
  Example:
          ...
          scrobbledb query rows plays --limit 20
          scrobbledb query rows tracks -c artist_name -c track_title --limit 10
          scrobbledb query rows plays --where "timestamp > :date" -p date 2024-01-01
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "schema", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query schema [OPTIONS] [TABLES]...

  Show full schema for this database or for specified tables.

  Example:

          scrobbledb query schema
          scrobbledb query schema tracks plays
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "search", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query search [OPTIONS] DBTABLE Q

  Execute a full-text search against this table.

  Example:

          scrobbledb query search tracks "rolling stones" --limit 10
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "tables", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query tables [OPTIONS]

  List the tables in the database.

  Example:

          scrobbledb query tables --counts --columns
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "triggers", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query triggers [OPTIONS] [TABLES]...

  Show triggers configured in this database.

  Example:

          scrobbledb query triggers
...
from click.testing import CliRunner
from retrocast.cli import cli
result = CliRunner().invoke(cli, ["query", "views", "--help"])
cog.out("```\n{}\n```".format(result.output))
]]] -->
```
Usage: cli query views [OPTIONS]

  List the views in the database.

  Example:

          scrobbledb query views --counts
