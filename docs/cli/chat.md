...
The `chat` command starts an interactive AI-powered REPL session for exploring your podcast transcription archive using natural language queries.
...
The chat feature requires optional dependencies:

```bash
pip install retrocast[chat]
```
...
```bash
retrocast chat --database /path/to/database.db
```
...
```bash
retrocast chat --model claude-3-5-sonnet-20241022
```
